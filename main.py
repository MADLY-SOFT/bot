...
def get_user_click_count(user_id: int) -> int:
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute('SELECT click_count FROM users WHERE user_id = ?', (user_id,))
    row = cursor.fetchone()
    conn.close()
    if row:
        return row[0]
...
def increment_user_click_count(user_id: int):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
async def handle_start(message: types.Message):
    ...
    increment_user_click_count(user_id)
